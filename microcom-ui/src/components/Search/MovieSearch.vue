<template>
    <div class="bar">
        <input id="searchbox" v-model="searchBoxRef" class="searchbar" type="text" placeholder="搜索">
        <q-select class="select" rounded standout v-model="searchIndex" :options="options"/>
    </div>
...
const initInstantSearch = () => {
                ...
                <div class="movie_card" id="bright">
                  <div class="info_section">
                      <div class="movie_header">
                          <img class="locandina"
                              src=${hit.poster} />
                          <p class="title" style="font-weight:900;font-size:2rem">${hit.title}</p>
                          <span class="minutes">${(new Date(hit.release_date)).toDateString()}</span>
                          <p class="type" style="font-size:15px;margin-top:20px">${(hit.genres).join('  ')}</p>
                      </div>
                  ...
                  <div class="blur_back" style="background-image: url('${hit.poster}')"></div>
                </div>
              </div>
            `},
            },
    ...
    watch(() => searchBoxRef.value, (newValue) => {
        // 监听搜索框的值变化
